...
Configurar en `Android-SDK-QA` → Settings → Secrets and variables → Actions:

| Tipo | Nombre | Para qué |
|---|---|---|
| Secret | `ANTHROPIC_API_KEY` | agentes `claude -p` headless (o ponerla en `scripts/.env` del runner) |
...
> **No se necesita ningún token de GitHub aquí.** El runner self-hosted corre como tu usuario, que
> ya tiene `gh` autenticado (scope `repo`): con ese login el pipeline **lee el diff del repo del SDK
> y crea el PR en el repo QA**. El workflow a propósito NO setea `GH_TOKEN`/`GITHUB_TOKEN` (si lo
> hiciera, `gh` usaría el token efímero del repo QA y no podría leer el repo del SDK).
